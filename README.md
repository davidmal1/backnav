...
So it still switches windows — except a browser with thirty tabs open is
...
*and* the tabs inside them, and gives you one shortcut to walk back along
it, and another to walk forward.
...
| --- | --- |
| **Tap** the shortcut | Step back one place. Repeat to keep going. No UI — you just watch windows and tabs come back. |
...
have. It cannot be supported, for the reason in the next section but one.
...
nothing else** — no URLs, no page content, no history — and every build
...
copy" — which is genuine, since every app on the supported list was
worked out by probing a live instance rather than by reading
documentation.
