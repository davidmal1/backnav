...
//
// Best-effort, and known to be dropped: the worker frequently respawns ON
// the onRemoved event, so connect() has only just run and the socket is
// still CONNECTING when this fires. reportLiveTabs() below is what makes
// that survivable.
...
// Observed live (2026-08-12) as a closed Brave tab still listed in the
// chooser.
//
// Also covers the cases nothing else does: closures during daemon
// downtime, and tab ids changed underneath us by Chrome's memory-saver
// discard (tabs.onReplaced fires instead of onRemoved, and nothing
// listens for it - the old id simply stops existing).
